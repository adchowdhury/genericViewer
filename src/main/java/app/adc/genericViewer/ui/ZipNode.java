...
public class ZipNode {
	...
	/**
	 * returns the last component of the name of the entry, i.e. the file name. If
	 * this is a directory node, the name ends with '/'. If this is the root node,
	 * the name is simply "/".
	 */
	public String getName() {
		...
		return longName.substring(longName.lastIndexOf('/', longName.length() - 2) + 1);
	}
	 ...
	 *
	 * The nodes' {@link #openStream()} methods are only usable until the ZipFile is
	 * closed, but the structure information remains valid.
	 */
	...
	private static class ZipFileReader {
		...
		/**
		 * reads all entries, creates the corresponding Nodes and returns the root node.
		 */
		ZipNode process() {
		 ...
		 *
		 * This may create a new ZipNode and then connects it to its parent node.
		 * 
		...
		/**
		 * makes sure that the parent of a node is in the collected-list as well, and
		 * this node is registered as a child of it. If necessary, the parent node is
		 * first created and added to the tree.
		 */
		 ...
		 */
		private void connectParent(ZipNode parent, ZipNode child, String childName) {
			if(parent == null || child == null || childName == null) {
				return;
			}
			child.parent = parent;
	 ...
	 */
	private void printTree(String prefix, String self, String sub) {
		System.out.println(prefix + self + this.getName());
		String				subPrefix	= prefix + sub;
		// the prefix strings for the next level.
		String				nextSelf	= " ├─ ";
		String				nextSub		= " │ ";
		Iterator<ZipNode>	iterator	= this.getChildren().values().iterator();
